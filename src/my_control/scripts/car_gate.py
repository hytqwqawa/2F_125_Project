...
class ImpactTimeControlGuidance:
    def __init__(self):
        ...
        self.save_csv = rospy.get_param('~save_csv', False)  # 获取 CSV 记录开关状态
        ...
        # 兼容原 launch 文件参数。新版 S0 选择不再使用该比例因子。
        ...
        # 角度误差恢复保护：只在 chi 几乎耗尽且角误差仍明显时平滑掺入
        ...
        if self.save_csv:
            ...
            filename = datetime.now().strftime("itacg_data_{}_%Y%m%d_%H%M%S.csv".format(self.car_name))
        ...
        rospy.loginfo(
            "SBHAG节点初始化完毕 | 车辆: %s | 目标点: (%.2f, %.2f) | T=%.2fs | theta_g=%.1fdeg | CSV记录: %s",
            self.car_name, self.target_x, self.target_y, base_impact_time, lambda_deg, self.save_csv
        )
    ...
    def run(self):
        ...
        if S_lower > S_upper:
            ...
            rospy.loginfo(
                "S0新版选择: 区间[%.2f, %.2f]m | eta_S=%.2f | S0=%.2f m",
        ...
        rospy.loginfo(
            "初始状态: R0=%.3f | sigma0=%.2fdeg | lambda_e0=%.2fdeg | chi0=%.3f | Ks=%.3f",
            R_0, math.degrees(sigma_0), math.degrees(lambda_e_0), chi_0, self.K_s
        )
