...
class ImpactTimeControlGuidance:
    def __init__(self):
        ...
        # 2. 制导律参数与平滑滤波设置
        self.desired_impact_time = 20.0- self.accel_duration
        ...
        self.dist_duration = 1.0             # 扰动持续时间 (秒)
        ...
        rospy.loginfo("SBHAG节点初始化完毕 | 启用高阶端点保护与偏航角锁死逻辑 | 开启CSV记录 | 开启扰动测试")
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == self.STATE_ACCEL:
                if t >= self.accel_duration:
                    ...
                    telemetry.update({'speed_desired': V_cmd, 'sigma_desired': 0, 'angular_vel': final_omega})
                ...
                # [扰动测试] 终段若时间恰好重合也叠加扰动量
                final_omega = self.filtered_omega + dist_omega
                self.publish_cmd(V_cmd, final_omega, dt_real)
                
                telemetry.update({'speed_desired': V_cmd, 'sigma_desired': 0, 'angular_vel': final_omega, 'omega_cmd': self.last_cmd_omega})
                self.publish_telemetry(telemetry)
                self.record_csv(t, telemetry)
