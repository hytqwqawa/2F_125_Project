...
class ImpactTimeControlGuidance:
    def __init__(self):
        rospy.init_node('impact_time_control_guidance_reverse', anonymous=True)

        # 0. Launch 参数与车辆名称
        self.car_name = rospy.get_param('~car_name', 'car2')
        self.save_csv = rospy.get_param('~save_csv', False)

        self.target_x = rospy.get_param('~target_x', 2.0)
        self.target_y = rospy.get_param('~target_y', 1.6)

        self.cruise_speed = rospy.get_param('~cruise_speed', 0.4)
        self.terminal_speed = rospy.get_param('~terminal_speed', 0.2)

        self.distance_budget_factor = rospy.get_param('~distance_budget_factor', 1.4)
        ...
        self.chi_min = rospy.get_param('~chi_min', 0.10)
        self.eta_min = rospy.get_param('~eta_min', 0.10)
        self.eta_max = rospy.get_param('~eta_max', 0.60)
        self.sigma_sat = math.radians(rospy.get_param('~sigma_sat_deg', 120.0))
        ...
        # 2. 制导律参数与平滑滤波设置
        base_impact_time = rospy.get_param('~desired_impact_time', 14.0)
        self.desired_impact_time = base_impact_time - self.accel_duration

        lambda_deg = rospy.get_param('~lambda_desired_deg', 90.0)
        self.lambda_desired = math.radians(lambda_deg)
        ...
        if self.save_csv:
            ...
            filename = datetime.now().strftime("itacg_reverse_data_{}_%Y%m%d_%H%M%S.csv".format(self.car_name))
        ...
        pose_topic = '/vrpn_client_node/{}/pose'.format(self.car_name)
        odom_topic = '/{}/odom'.format(self.car_name)
        cmd_topic = '/{}/cmd_vel'.format(self.car_name)
        ...
        self.pubs = {
            name: rospy.Publisher('/itacg_{}/{}'.format(self.car_name, name), Float64, queue_size=10)
            for name in topics
        }
        self.rate = rospy.Rate(50)

        rospy.loginfo(
            "SBHAG[倒车模式]初始化完毕 | 车辆: %s | 目标点: (%.2f, %.2f) | T=%.2fs | theta_g=%.1fdeg",
            self.car_name, self.target_x, self.target_y, base_impact_time, lambda_deg
        )

    def shutdown_hook(self):
        if self.save_csv and not self.csv_file.closed:
            self.csv_file.close()
    ...
    def publish_cmd(self, v, omega, dt):
        ...
        cmd = Twist()
        # =====================================================================
        # [倒车核心修改 1/2] 算法层按正值计算，执行层强制取反实现倒退
        cmd.linear.x = -v  
        # =====================================================================
    ...
    def run(self):
        pose_topic = '/vrpn_client_node/{}/pose'.format(self.car_name)
        rospy.loginfo("等待动捕数据 (%s) ...", pose_topic)
        try:
            self.pose_callback(rospy.wait_for_message(pose_topic, PoseStamped, timeout=5.0))
        except rospy.ROSException:
            return rospy.logerr("未收到动捕数据，节点退出")

        # =====================================================================
        # [倒车核心修改 2/2] 获取有效航向 (即车尾的朝向) 用于控制算法
        effective_yaw_0 = self.normalize_angle(self.current_yaw + math.pi)
        # =====================================================================

        R_0 = math.hypot(self.target_x - self.current_x, self.target_y - self.current_y)
        los_0 = self.get_line_of_sight_angle()
        sigma_0 = self.normalize_angle(effective_yaw_0 - los_0)
        ...
        # Dubins 规划中传入有效航向 effective_yaw_0
        L_min = self.dubins_length(
            self.current_x, self.current_y, effective_yaw_0,
        ...
        if abs(chi_0) > 1e-4:
            self.K_s = abs((R_0 * lambda_e_0 / chi_0) * math.tan(sigma_0 / 2.0))
        else:
            self.K_s = self.K_f
        ...
        while not rospy.is_shutdown():
            ...
            # 每帧提取有效航向
            effective_yaw = self.normalize_angle(self.current_yaw + math.pi)
            sigma = self.normalize_angle(effective_yaw - los)
            ...
            if self.state == self.STATE_ACCEL:
                if t >= self.accel_duration:
                    ...
                    telemetry.update({'speed_desired': -V_cmd, 'sigma_desired': 0.0, 'angular_vel': 0.0})
                ...
                V_cmd = self.v_0 + (self.v_g - self.v_0) * (3.0 * s ** 2 - 2.0 * s ** 3) + self.c_coeff * (s ** 2 * (1.0 - s) ** 2)
                ...
                lambda_reg = lambda_e if abs(lambda_e) > 0.01 else math.copysign(0.01, lambda_e)
                denom = max(R, 0.05) * lambda_reg
                ...
                telemetry.update({
                    'speed_desired': -V_cmd,  # Telemetry中保存反向速度方便查阅
                ...
                # 终段锁定同样使用 effective_yaw 比较
                yaw_error = self.normalize_angle(self.locked_yaw - effective_yaw)
