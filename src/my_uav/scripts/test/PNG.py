...
"""
PN guidance without yaw maneuver, using analytic LOS-rate.

Task:
1) Arm and switch to CMD_CONTROL.
2) Take off and hover.
3) Fly toward a fixed target using a proportional-navigation-like
   velocity-direction law.
4) No yaw maneuver is commanded: yaw-rate command is zero.
5) The horizontal speed magnitude is fixed at 0.3 m/s.
6) Altitude is held at the hover altitude by XY_VEL_Z_POS.
...
Example sign check:
    ...
    Then lambda_dot = -V/R < 0.
    This is correct because the bearing from vehicle to target rotates clockwise.
"""
...
class PNBodyVelocityAnalyticLOSNode:
    def __init__(self):
        rospy.init_node("pn_body_velocity_analytic_los", anonymous=False)
        ...
        self.speed = rospy.get_param("~speed", 0.3)                 # constant horizontal speed [m/s]
        self.N = rospy.get_param("~nav_gain", 3.0)                  # PN navigation constant
        self.max_course_rate = rospy.get_param("~max_course_rate", 0.8)  # [rad/s]
        self.land_radius = rospy.get_param("~land_radius", 0.10)    # trigger land when R < this value
        self.timeout = rospy.get_param("~timeout", 60.0)            # maximum guidance duration [s]
        ...
        # Actual horizontal velocity in ENU. It is updated from UAVState.velocity.
        ...
        rospy.loginfo("pn_body_velocity_analytic_los started.")
        rospy.loginfo("uav_name = %s", self.uav_name)
        rospy.loginfo("target = (%.2f, %.2f), speed = %.2f m/s, N = %.2f",
                      self.target_x, self.target_y, self.speed, self.N)
    ...
    def pn_guidance_loop(self, z_hold):
        rospy.loginfo("start PN guidance without yaw maneuver.")
        ...
        # This makes the velocity vector bend gradually through PN.
        self.gamma_cmd = yaw

        # Initialize command velocity fallback.
        ...
        while not rospy.is_shutdown():
            ...
            # Analytic LOS-rate with carefully checked sign:
            # lambda_dot = (ry * vx - rx * vy) / R^2.
            ...
            # A small capture term helps avoid poor convergence when LOS-rate is tiny.
            # Set _los_capture_gain:=0.0 if a stricter PN-only behavior is desired.
            ...
            # Use XY_VEL_Z_POS to hold altitude. No yaw maneuver: yaw-rate is zero.
            cmd = self.make_cmd()
            ...
            rospy.loginfo_throttle(
                1.0,
                "R=%.2f, LOS=%.1f deg, los_dot=%.3f rad/s, gamma=%.1f deg, "
...
if __name__ == "__main__":
    try:
        PNBodyVelocityAnalyticLOSNode().run()
